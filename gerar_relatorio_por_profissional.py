import re
from pathlib import Path
from models.report_generator import ReportDataExtractor
from datetime import datetime
import json
import os
from models.report_generator import ReportGenerator
from models.storage import sanitize_profissional_name

CPF = '43795607833'


def find_profissional_folder(cpf: str):
    root = Path(__file__).resolve().parent
    prof_file = root / 'profissionais.json'
    if prof_file.exists():
        with open(prof_file, 'r', encoding='utf-8') as f:
            data = json.load(f)
        for prof in data.get('profissionais', []):
            if str(prof.get('cpf_prof', '')).replace('.', '').replace('-', '') == cpf:
                folder_name = sanitize_profissional_name(prof.get('apelido', ''))
                candidate = root / 'profissionais' / folder_name
                if candidate.exists():
                    return candidate
    for folder in (root / 'profissionais').glob('*'):
        if folder.is_dir():
            recibos_file = folder / 'recibos_saude.json'
            pacientes_file = folder / 'pacientes.json'
            if recibos_file.exists() or pacientes_file.exists():
                try:
                    if recibos_file.exists():
                        with open(recibos_file, 'r', encoding='utf-8') as f:
                            recibos = json.load(f).get('recibos', [])
                        if any(r.get('cpf_pagador') == cpf or r.get('cpf_benef') == cpf for r in recibos):
                            return folder
                    if pacientes_file.exists():
                        with open(pacientes_file, 'r', encoding='utf-8') as f:
                            pacientes = json.load(f).get('pacientes', [])
                        if any(p.get('cpf_pagador') == cpf or p.get('cpf_benef') == cpf for p in pacientes):
                            return folder
                except Exception:
                    continue
    return root / 'profissionais'


professional_dir = find_profissional_folder(CPF)
recibos_path = professional_dir / 'recibos_saude.json'
if not recibos_path.exists():
    print('Recibos não encontrados na pasta do profissional')
    exit(1)

with open(recibos_path, 'r', encoding='utf-8') as f:
    data = json.load(f)
    recibos = data.get('recibos', [])

recibo = None
for r in recibos:
    if r.get('cpf_pagador') == CPF or r.get('cpf_benef') == CPF:
        recibo = r
        break

if not recibo:
    print('Recibo não encontrado')
    exit(1)

# Carregar paciente
paciente = None
pacientes_path = professional_dir / 'pacientes.json'
if pacientes_path.exists():
    try:
        with open(pacientes_path, 'r', encoding='utf-8') as pf:
            pd = json.load(pf)
        for p in pd.get('pacientes', []):
            if p.get('cpf_pagador') == CPF or p.get('cpf_benef') == CPF:
                paciente = p
                break
    except Exception:
        pass

if not paciente:
    print('Paciente não encontrado')
    exit(1)

# Definir template existente (com underscore)
template = os.path.join('modelosRelatorios',
                        '_RelatorioTemplateCarimboFem.docx')
if not os.path.exists(template):
    print('Template não encontrado:', template)
    exit(1)

# Definir pasta de saída (Desktop/Relatório de Maio etc.)
variables = ReportGenerator(
    template).replace_text_in_document if False else None
# Use ReportGenerator to build filename similar ao UI
vars = ReportDataExtractor.extract_report_variables(recibo, paciente)
nome_benef = vars.get('#NomePac', 'Paciente')
ano = vars.get('#AnoDasConsultas2', '')
mes = vars.get('#MesDasConsultas2', '')
if not mes:
    mes = datetime.now().strftime('%B')
mes_cap = mes.capitalize()
output_folder = os.path.expanduser('~/Desktop')
pasta_mes = f'Relatório de {mes_cap}'
caminho_mes = os.path.join(output_folder, pasta_mes)
if not os.path.exists(caminho_mes):
    os.makedirs(caminho_mes, exist_ok=True)

nome_limpo = re.sub(r'[<>:"/\\|?*]', '', nome_benef).strip()
output_filename = f"{nome_limpo} - Relatório {ano}{mes.capitalize()}.pdf"
output_path = os.path.join(caminho_mes, output_filename)

print('Gerando em:', output_path)

rg = ReportGenerator(template)
success = rg.generate_report(recibo, paciente, output_path)
print('Sucesso:', success)
